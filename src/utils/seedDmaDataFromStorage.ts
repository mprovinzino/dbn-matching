...
// This utility populates the zip_code_reference table with all DMA data
...
function inferState(zipCode: string): string {
  const prefix = zipCode.substring(0, 3);
  return ZIP_PREFIX_TO_STATE[prefix] || 'Unknown';
}

export async function seedDmaData() {
  console.log('Starting DMA data seed...');
  
  // Call the edge function to populate data
  // Note: This needs to be called with an Excel file via the import button
  // Or we can create the data directly here
  
  const toast = document.createElement('div');
  toast.textContent = 'Please use the "Import DMA Data" button with your Excel file';
  toast.style.cssText = 'position: fixed; top: 20px; right: 20px; background: #333; color: white; padding: 16px; border-radius: 8px; z-index: 9999;';
  document.body.appendChild(toast);
  setTimeout(() => toast.remove(), 5000);
  
  return { message: 'Use Import DMA Data button to import from Excel file' };
}
