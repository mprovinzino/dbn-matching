...
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '320': 'FL', '321': 'FL', '322': 'FL', '323': 'FL', '324': 'FL', '325': 'FL', '326': 'FL', '327': 'FL', '328': 'FL', '329': 'FL', '330': 'FL', '331': 'FL', '332': 'FL', '333': 'FL', '334': 'FL', '335': 'FL', '336': 'FL', '337': 'FL', '338': 'FL', '339': 'FL', '340': 'FL', '341': 'FL', '342': 'FL',
  '344': 'FL',
  '346': 'FL', '347': 'FL',
  '349': 'FL',
...
export async function importDmaZipCodes(file: File) {
  try {
    const workbook = XLSX.read(await file.arrayBuffer());
    const sheet = workbook.Sheets[workbook.SheetNames[0]];
    const rows = XLSX.utils.sheet_to_json(sheet);
    
    const zipCodeData = rows
      .filter((row: any) => row.zip_code && row.dma_description)
      .map((row: any) => ({
        zip_code: String(row.zip_code).padStart(5, '0'),
        city: 'Unknown',
        state: inferState(String(row.zip_code).padStart(5, '0')),
        dma: row.dma_description,
        latitude: null,
        longitude: null,
      }));
