-- Patch get_dma_coverage_density to only include full_coverage in state_based branch to avoid double-counting and phantom investors
...
BEGIN
  ...
  state_based AS (
    ...
    WHERE zcr.dma IS NOT NULL 
      ...
      AND m.market_type = 'full_coverage'  -- ensure only national/statewide coverage flows through this branch
      AND i.status = 'active'
  ),
  dma_investors AS (
