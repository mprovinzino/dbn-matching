...
// State inference from zip prefix
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  '939': 'CA', '940': 'CA', '941': 'CA', '942': 'CA', '943': 'CA', '944': 'CA', '945': 'CA', '946': 'CA', '947': 'CA',
  '948': 'CA', '949': 'CA', '950': 'CA', '951': 'CA', '952': 'CA', '953': 'CA', '954': 'CA', '955': 'CA', '956': 'CA',
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Clear existing data (except placeholder)
    console.log('Clearing existing zip code reference data...');
    ...
    // Process and prepare records for insertion
    const records = dmaData.map((entry: any) => {
      const zipCode = String(entry.zip_code).padStart(5, '0');
      const prefix = zipCode.substring(0, 3);
      const state = ZIP_PREFIX_TO_STATE[prefix] || 'XX';
      
      return {
        zip_code: zipCode,
        city: 'Unknown',
        state: state,
        dma: entry.dma_description,
        latitude: null,
        longitude: null
      };
    });
    ...
    const { data: uniqueDmas, error: countError } = await supabaseAdmin
      .from('zip_code_reference')
      .select('dma')
      .not('dma', 'is', null);
    
    const dmaCount = uniqueDmas ? new Set(uniqueDmas.map(d => d.dma)).size : 0;
    
    console.log(`Successfully imported ${imported} zip codes across ${dmaCount} unique DMAs`);
    
    return new Response(
      JSON.stringify({
        success: true,
        imported: imported,
        unique_dmas: dmaCount,
        message: `DMA reference data loaded successfully: ${imported} zip codes across ${dmaCount} DMAs`
      }),
      { 
